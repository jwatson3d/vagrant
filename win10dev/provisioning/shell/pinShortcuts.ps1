...
#$pn = $sa.namespace("${env:ProgramFiles(x86)}").parsename('Microsoft Visual Studio 14.0\Common7\IDE\devenv.exe'); $pn.invokeverb('taskbarpin')
...
$pn = $sa.namespace("${env:ProgramFiles(x86)}").parsename('Notepad++\notepad++.exe'); $pn.invokeverb('taskbarpin'); $pn = $sa.namespace("${env:ProgramFiles(x86)}").parsename('ChocolateyGUI\ChocolateyGui.exe'); $pn.invokeverb('taskbarpin')
